...
var grammar = {
    ...
    {"name": "identifier$ebnf$1", "symbols": [(lexer.has("word") ? {type: "word"} : word)], "postprocess": id},
    {"name": "identifier$ebnf$1", "symbols": [], "postprocess": function(d) {return null;}},
    {"name": "identifier$ebnf$2", "symbols": []},
    {"name": "identifier$ebnf$2$subexpression$1", "symbols": [(lexer.has("comma") ? {type: "comma"} : comma), (lexer.has("word") ? {type: "word"} : word)]},
    {"name": "identifier$ebnf$2", "symbols": ["identifier$ebnf$2", "identifier$ebnf$2$subexpression$1"], "postprocess": function arrpush(d) {return d[0].concat([d[1]]);}},
    {"name": "identifier", "symbols": [(lexer.has("lparen") ? {type: "lparen"} : lparen), "identifier$ebnf$1", "identifier$ebnf$2", (lexer.has("rparen") ? {type: "rparen"} : rparen), (lexer.has("thinarrow") ? {type: "thinarrow"} : thinarrow), "expression"], "postprocess":  (data) => ( { type:'lambda', ident: data[0],func:data[1], 
        args: [data[1], ...(data[2].flat().filter(a=>a.type!=='comma') ) ],
        expression: data[5] } ) },
    {"name": "identifier", "symbols": ["expression", (lexer.has("word") ? {type: "word"} : word), "expression"], "postprocess":  (data) => ( { type:'fun-call',  fun:data[1], 
