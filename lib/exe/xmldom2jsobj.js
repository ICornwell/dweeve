...
function hasText(node) {
  ...
  for (var idx = 0; idx < node.childNodes.length; idx++) {
    if (node.childNodes.item(idx).splitText && !/^\s*$/.test(node.childNodes.item(idx).textContent)) return true;
  }
...
function nodeOwnText(node) {
  ...
  for (var idx = 0; idx < node.childNodes.length; idx++) {
    if (node.childNodes.item(idx).splitText && !/^\s*$/.test(node.childNodes.item(idx).textContent)) return node.childNodes.item(idx).textContent;
  }
...
function isTextOnlyElement(node) {
  return getNodeType(node) === 'Element' && node.childNodes.length == 1 && node.firstChild.splitText;
}
